...
<body style="margin:0;padding:0;background:#070f1a;font-family:Arial,sans-serif;color:#e2e8f0;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:#052e16;border:2px solid #16a34a;border-radius:16px;padding:20px;margin-bottom:20px;text-align:center;">
      <p style="margin:0 0 4px;font-size:22px;font-weight:900;color:#4ade80;">✅ MOKĖJIMAS PATVIRTINTAS</p>
      <p style="margin:0;color:#86efac;font-size:14px;">💎 Užsakymas #${order.order_number} — tikslus mokėjimas gautas</p>
    </div>` : `
    <div style="background:#1e3a8a;border:2px solid #3b82f6;border-radius:16px;padding:20px;margin-bottom:20px;text-align:center;">
      <p style="margin:0 0 4px;font-size:22px;font-weight:900;color:#93c5fd;">🆕 NAUJAS UŽSAKYMAS</p>
      <p style="margin:0;color:#bfdbfe;font-size:14px;">📦 Užsakymas #${order.order_number} — laukiama mokėjimo</p>
    </div>`}
    ...
    <div style="background:#0a1929;border:1px solid #1e3a5f;border-radius:16px;padding:24px;margin-bottom:20px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        ...
        <span style="background:${isPaymentConfirmed ? '#16a34a' : '#1e3a5f'};color:${isPaymentConfirmed ? '#bbf7d0' : '#93c5fd'};padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700;">${isPaymentConfirmed ? '✅ APMOKĖTA' : '🆕 NAUJAS'}</span>
      </div>
    ...
    <div style="background:#0a1929;border:1px solid #1e3a5f;border-radius:16px;padding:24px;margin-bottom:20px;">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tr><td style="padding:6px 0;color:#64748b;">🏙️ Miestas:</td><td style="padding:6px 0;color:#fff;">${escapeHtml(order.customer_city) || "—"}</td></tr>
        ...
        <tr><td style="padding:6px 0;color:#64748b;">🎟️ Nuolaidos kodas:</td><td style="padding:6px 0;${pricing.discount_code ? 'color:#4ade80;font-weight:700;' : 'color:#fff;'}">${escapeHtml(pricing.discount_code) || "—"}</td></tr>
    ...
    <div style="background:#052e16;border:1px solid #166534;border-radius:12px;padding:16px;margin-bottom:20px;">
      <p style="margin:0 0 10px;color:#4ade80;font-weight:700;">🎟️ Panaudotas nuolaidos kodas: ${escapeHtml(pricing.discount_code)} (${escapeHtml(pricing.discount_percent)}% nuolaida, sutaupyta ${Number(pricing.discount_amount_eur || 0).toFixed(2)}€)</p>
    ...
    <div style="background:#0a1929;border:1px solid #1e3a5f;border-radius:16px;padding:20px;margin-bottom:20px;">
      <h3 style="margin:0 0 12px;font-size:13px;color:#64748b;text-transform:uppercase;letter-spacing:1px;">⛓️ Solana mokėjimo informacija</h3>
      ...
      emailRes = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          subject: isPaymentConfirmed
            ? `✅ APMOKĖTA #${order.order_number} — ${Number(order.total_amount).toFixed(2)}€ gauta!`
